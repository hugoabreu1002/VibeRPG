...
const calculateEvolutionGains = (character: Character) => {
  ...
  const statBoost = {
    ...
    hp: Math.floor(character.maxHp * 0.15) + 10,
    maxMp: Math.floor(character.maxMp * 0.15) + 5,
    mp: Math.floor(character.maxMp * 0.15) + 5,
    ...
    speed: Math.floor(character.speed * 0.15) + 2,
  };
...
export function GuildEvolution({ character, onEvolve, onClose }: GuildEvolutionProps) {
  ...
  const handleEvolve = () => {
    if (!evolutionData?.nextRank || !evolutionCheck.can) return;

    audioManager.playSfx("victory");

    const newSkills = getRankSkills(character.class, evolutionData.nextRank.rank);
    ...
    const updatedCharacter: Character = {
      ...character,
      rank: evolutionData.nextRank.rank,
      skills: newSkills,
      ...statBoost,
    };
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-amber-200" style={{ fontFamily: "'Cinzel', serif" }}>Guild Evolution</h2>
          <p className="text-sm text-slate-400">Ascend to higher ranks and unlock new powers</p>
        </div>
        <button
          onClick={onClose}
          ...
          {evolutionData?.nextRank ? (
            <>
              ...
              >
                ...
                <div className="flex flex-col items-center py-4">
                  <motion.div
                    animate={{
                      scale: [1, 1.1, 1],
                      filter: [`drop-shadow(0 0 10px ${evolutionData.nextRank.color}44)`, `drop-shadow(0 0 25px ${evolutionData.nextRank.color}88)`, `drop-shadow(0 0 10px ${evolutionData.nextRank.color}44)`]
                    }}
                    transition={{ duration: 3, repeat: Infinity }}
                    className="text-7xl font-black mb-2"
                    style={{ color: evolutionData.nextRank.color }}
                  >
                    {evolutionData.nextRank.rank}
                  </motion.div>
                  <span className="text-xl font-bold text-white">{evolutionData.nextRank.label}</span>
                </div>

                <div className="mt-6 p-4 rounded-xl bg-black/40 border border-white/5 space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-400">Minimum Level</span>
                    <span className={`text-base font-bold flex items-center gap-1.5 ${character.level >= evolutionData.nextRank.minLevel ? 'text-emerald-400' : 'text-red-400'}`}>
                      Level {evolutionData.nextRank.minLevel}
                    </span>
                  </div>
                </div>
                <div className="mt-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <span className="text-xs py-1 px-3 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-200">New Skills</span>
                    <span className="text-xs py-1 px-3 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-200">Stat Boost</span>
                    <span className="text-xs py-1 px-3 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-200">Elite Sprite</span>
                  </div>
                </div>
                ...
                <motion.button
                  ...
                  className={`w-full mt-8 py-4 rounded-xl font-black uppercase tracking-widest transition-all shadow-xl ${evolutionCheck.can
                ...
                {/* Evolution Gains Preview */}
                {evolutionData && evolutionCheck.can && (
                  <div className="mt-6 p-4 rounded-xl bg-amber-500/10 border border-amber-500/20">
                    <h4 className="text-[10px] font-bold text-amber-500/60 uppercase tracking-widest mb-4">Evolution Gains</h4>
                    <div className="space-y-3">
                      <div className="flex justify-between text-sm">
                        <span className="text-slate-400">New Rank</span>
                        <span className="text-amber-300 font-bold">{evolutionData.nextRank.rank}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-slate-400">New Skills</span>
                        <span className="text-amber-300 font-bold">{evolutionData.newSkills.length}</span>
                      </div>
                      <div className="grid grid-cols-4 gap-2">
                        {Object.entries(evolutionData.statBoost).map(([stat, boost]) => (
                          <div key={stat} className="text-xs text-slate-400">
                            <div className="font-bold text-amber-300">{boost > 0 ? `+${boost}` : boost}</div>
                            <div className="text-xs text-slate-400 uppercase">{stat}</div>
                          </div>
                        ))}
                      </div>
                    </div>
              ...
              <div
                ...
                style={{ background: `radial-gradient(circle at center, ${evolutionData.nextRank.color} 0%, transparent 70%)` }}
              />
            </>
          ) : (
            <div className="flex flex-col items-center justify-center h-full text-center">
              <div className="text-5xl mb-4">👑</div>
              <h3 className="text-xl font-bold text-amber-200 mb-2">Maximum Rank</h3>
              <p className="text-sm text-slate-400">You have reached the pinnacle of power</p>
            </div>
          )}
        </div>
      </div>
